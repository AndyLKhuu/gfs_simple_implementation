...
type MasterServer struct {
	...
	filesLock          sync.RWMutex         // RWLock for Files
	chunkLock          sync.RWMutex         // RWLock for Chunks
	leases             map[uint64]leaseInfo // ChunkHandles to leaseInfo
	chunkHandleSet     map[uint64]bool      // Set of used chunkhandles
	chunkHandleSetLock sync.RWMutex         // RWLock for chunk handle set
}
...
func NewMasterServer() *MasterServer {
	return &MasterServer{
		...
		chunkHandleSetLock: sync.RWMutex{}}
}
...
func (s *MasterServer) GetChunkLocation(ctx context.Context, chunkLocReq *protos.ChunkLocationRequest) (*protos.ChunkLocationReply, error) {
	...
	_, ok := s.Files[path]
	if !ok {
		return &protos.ChunkLocationReply{}, errors.New("Chunk does not exist")
	}
...
func (s *MasterServer) CreateFile(ctx context.Context, createReq *protos.FileCreateRequest) (*protos.Ack, error) {
	...
	_, e := os.Stat(path)
	if e == nil {
		return &protos.Ack{Message: fmt.Sprintf("file already exists at path %s", path)}, nil
	}
...
func (s *MasterServer) RemoveFile(ctx context.Context, removeReq *protos.FileRemoveRequest) (*protos.Ack, error) {
	...
	if errors.Is(e, os.ErrNotExist) {
		return &protos.Ack{Message: fmt.Sprintf("file does not exist %s", path)}, nil
	}
